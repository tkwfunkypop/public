...
- **Claude Code on the web（リモート実行環境）では認証できない。**
  `.mcp.json` 自体はリモートセッションでも読み込まれるが、そこで止まる。実際に出るのは
  「`fish-audio` requires authentication / このセッションは非対話なのでOAuthフローを実行できない」という表示。
  リモートコンテナは外向き通信もネットワークポリシーで制限されていて、`api.fish.audio` への
  CONNECT が 403 で弾かれる。**認証はローカルの Claude Code（ターミナル / デスクトップアプリ）で行うこと。**
